// Bet API Route for O Racha - KASSINO-CKB
import { NextRequest, NextResponse } from "next/server";
import { supabase, getSupabaseAdmin } from "@/lib/supabase-client";

const VALID_VEHICLES = ["ford-ka", "purpple-horse", "green-horse", "yellow-horse", "blue-horse"];
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { betAmount, selectedVehicle } = body;
    ...
    if (!selectedVehicle || !VALID_VEHICLES.includes(selectedVehicle)) {
      return NextResponse.json({ error: "Veículo selecionado inválido" }, { status: 400 });
    }
    ...
    if (round.status !== "betting") {
      return NextResponse.json({ error: "As apostas para esta corrida já se encerraram!" }, { status: 400 });
    }
    ...
    if (existingBet) {
      return NextResponse.json({ error: "Você já realizou uma aposta nesta corrida!" }, { status: 400 });
    }
    ...
    console.error("Racha bet error:", err);
